package com.udacoding.drivergitfire.login
...
import com.udacoding.drivergitfire.auth.AutentikasiHpActivity
import com.udacoding.drivergitfire.network.myFirebaseDatabase
import com.udacoding.drivergitfire.signup.SignUpActivity
...
class LoginActivity : AppCompatActivity() {
    ...
    private fun checkDatabase(responseUser: FirebaseUser) {
        //  add query
        val query = myFirebaseDatabase.driverRef().orderByChild("uid").equalTo(responseUser.uid)
        ...
        query.addListenerForSingleValueEvent(object :ValueEventListener{
            ...
            override fun onDataChange(p0: DataSnapshot) {
                if(p0.value == null){
                    ...
                    key?.let { myFirebaseDatabase.driverRef().child(it).setValue(newUser) }
